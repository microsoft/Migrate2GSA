...
function Test-CSVValidation {
    ...
    for ($i = 0; $i -lt $CSVData.Count; $i++) {
        ...
        # Validate Protocol
        if (-not [string]::IsNullOrWhiteSpace($row.Protocol) -and $row.Protocol -notin $validProtocols) {
            $rowErrors += "Invalid Protocol: '$($row.Protocol)'. Must be one of: $($validProtocols -join ', ')"
        }
        ...
        if (-not [string]::IsNullOrWhiteSpace($row.Ports)) {
            $portPattern = '^(\d{1,5}(-\d{1,5})?)(,\d{1,5}(-\d{1,5})?)*$'
            if ($row.Ports -notmatch $portPattern) {
                $rowErrors += "Invalid port format: '$($row.Ports)'. Use format: '80', '80,443', '8000-8999'"
            }
        }
