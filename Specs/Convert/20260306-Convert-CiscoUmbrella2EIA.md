...
- Convert Umbrella application-based rules to EIA FQDN-based policies using a provided application mapping file
...
| Web rule (`application_ids`) | → | WCF Policy (FQDN rules) | Resolve via `extradata.applications[]`, look up in app mapping file |
...
### 6. CiscoUmbrella2EIA-AppMappings.json
**Source:** Manual configuration file (maintained by user)
**Required:** Yes when `web_policies.json` is provided (used to resolve application-based rules)

#### Description
Provides mapping between Cisco Umbrella application names and their FQDN/URL endpoints for use in EIA web content filtering policies.
...
#### Processing Rules — Two-Pass Lookup

For each Umbrella application name (resolved from `extradata.applications[]`):

1. **Exact match** (case-insensitive): Umbrella app label matches `displayName` exactly → use `endpoints[]`, no review flag
2. **Approximate match** (case-insensitive substring): If no exact match, check if either name contains the other (e.g., "Confluence" is contained in "Atlassian Confluence", or "Atlassian Confluence" contains "Confluence") → use `endpoints[]`, but **flag for review** with both names noted
3. **No match**: Application not found in mapping file → flag for review, no endpoints generated, log warning
...
| ReviewNeeded | Manual review flag | "Yes", "No" | "Yes" if unmapped, approximate match, or warn/isolate action |
| ReviewDetails | Reason for review | "Approximate app match: Confluence → Atlassian Confluence" | Semicolon-separated reasons |
...
7. If `WebPoliciesPath` is provided: Load `CiscoUmbrella2EIA-AppMappings.json` — Fatal error if missing or invalid JSON
...
# App mappings: displayName (lowercase) → endpoints[]
$appMappingsHashtable = @{}
foreach ($app in $appMappings) {
    $appMappingsHashtable[$app.displayName.ToLower()] = $app
}
...
For each web policy, build lookup tables from its `extradata`:
...
foreach ($appId in $applicationIds) {
    ...
    if ($null -eq $umbrellaApp) {
        ...
        $reviewReasons += "Unknown application ID: $appId"
        continue
    }
    ...
    $mappedApp = $appMappingsHashtable[$appName.ToLower()]
    
    if ($null -ne $mappedApp) {
        # Exact match found
        foreach ($endpoint in $mappedApp.endpoints) {
            $allEndpoints += $endpoint
            $allEndpoints += "*.$endpoint"
        }
    }
    else {
        # Pass 2: Approximate match (substring, case-insensitive)
        $approxMatch = $null
        foreach ($key in $appMappingsHashtable.Keys) {
            $mappingName = $appMappingsHashtable[$key].displayName
            if ($appName.ToLower().Contains($key) -or $key.Contains($appName.ToLower())) {
                $approxMatch = $appMappingsHashtable[$key]
                break
            }
        }
        
        if ($null -ne $approxMatch) {
            # Approximate match found — use endpoints but flag for review
            foreach ($endpoint in $approxMatch.endpoints) {
                $allEndpoints += $endpoint
                $allEndpoints += "*.$endpoint"
            }
            $reviewReasons += "Approximate app match: $appName -> $($approxMatch.displayName)"
        }
        else {
            # No match at all
            $reviewReasons += "No app mapping found: $appName"
            Write-LogMessage "No app mapping found for: $appName (ID: $appId)" -Level WARN
        }
    }
}

# Create FQDN policy if endpoints were resolved
if ($allEndpoints.Count -gt 0) {
    ...
    if ($rule.ruleAction -eq "warn") {
        $reviewReasons += "Original action was 'warn' (user click-through) — converted to Block"
    }
    if ($rule.ruleAction -eq "isolate") {
        $reviewReasons += "Original action was 'isolate' (remote browser isolation) — converted to Block"
    }
    $hasReview = $hasReview -or $rule.ruleAction -in @("warn", "isolate")
    
    $groups = Split-ByCharacterLimit -Entries $allEndpoints -MaxLength 300
...
Applications matched (approximate, flagged): D
Applications unmatched (flagged): E
...
| AppMappingsPath | string | `CiscoUmbrella2EIA-AppMappings.json` | Path to app mappings file | ValidateScript - file must exist. Required when WebPoliciesPath is provided |
...
param(
    ...
    [Parameter(HelpMessage = "Path to Umbrella to EIA application mappings JSON file")]
    [ValidateScript({
        if (Test-Path $_) { return $true }
        else { throw "File not found: $_" }
    })]
    [string]$AppMappingsPath = (Join-Path $PWD "CiscoUmbrella2EIA-AppMappings.json"),
...
#### 4. Find-AppMapping
**Purpose:** Look up an application name in a mappings hashtable using two-pass strategy (exact then approximate)

**Location:** `Migrate2GSA\internal\functions\Find-AppMapping.ps1` (shared internal function — reusable across converters)

**Returns:** Hashtable with `Endpoints`, `MatchType` ("exact", "approximate", "none"), and `MappedName`
...
function Find-AppMapping {
    ...
    foreach ($key in $AppMappingsHashtable.Keys) {
        $mappingName = $AppMappingsHashtable[$key].displayName
        $mappingNameLower = $mappingName.ToLower()
        
        if ($appNameLower.Contains($mappingNameLower) -or $mappingNameLower.Contains($appNameLower)) {
            return @{
                Endpoints = $AppMappingsHashtable[$key].endpoints
...
**Status:** New — to be created as a shared internal function. Two-pass lookup (exact then approximate substring match) against an app mappings hashtable keyed by `displayName.ToLower()`. See helper function #4 above for full logic.
...
| WARN | Skipped items, unmapped categories, missing app mappings, bypassed settings | "No app mapping found: Yandex Browser", "Advise configuring Threat Intel" |
...
| App not in mapping file | No match found | WARN, flag for review |
| Approximate app match | Substring match only | INFO, use endpoints, flag for review |
...
Web-AllowFileShare-Apps-Allow,WebContentFiltering,Allow,"Converted from Umbrella web rule: Allow - FileShare",FQDN,"fileshare.example;*.fileshare.example",FQDNs,Yes,"Approximate app match: FileShare Pro -> FileShare",no
```
...
- [ ] Create Find-AppMapping as shared internal function in `Migrate2GSA\internal\functions\Find-AppMapping.ps1`
...
- [ ] Convert application conditions via app mapping file (two-pass)
...
12. **App mapping completeness**: Depends on the quality of the user-provided app mapping file. Approximate matching may produce false positives.
...
5. Improve approximate app matching with configurable similarity threshold
